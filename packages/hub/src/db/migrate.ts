...
export function migrateHubDatabase(dbPath: string): void {
  ...
  try {
    ...
    // Check if sync_status exists in offers/requests
    ...
    console.log('[OK] Hub database migration complete');
  } catch (error: any) {
...
export function migrateHubIfDatabaseExists(dbPath?: string): void {
  const finalPath = dbPath || path.join(__dirname, '../../data/hub.db');
  const exists = require('fs').existsSync(finalPath);

  if (exists) {
    migrateHubDatabase(finalPath);
  }
}
